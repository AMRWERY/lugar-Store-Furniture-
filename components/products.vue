        ...
        <div class="space-y-12 lg:grid lg:grid-cols-3 lg:gap-x-6 lg:space-y-0">
          <nuxt-link :to="`/all-products/${product.id}`" v-for="product in paginatedProducts" :key="product.id" class="relative group">
            ...
            <div class="flex items-center mt-2 space-s-1">
              <p class="text-lg font-semibold text-gray-900">{{ product.discountedPrice }} LE</p>
              <p class="text-sm font-semibold text-gray-500 line-through">{{ product.originalPrice }} LE</p>
            </div>
          </nuxt-link>
        </div>
      </div>
